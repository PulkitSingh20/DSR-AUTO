...
// ─── Audit Helper ─────────────────────────────────────────────────────────────
export async function auditLog(table: string, recordId: string, action: string, changedBy = "system", changes?: any) {
  try {
...
export const shipmentRepo = {
  ...
  async create(data: any, userId = "system") {
  ...
  async update(id: string, data: any, userId = "system") {
    const updated = await ShipmentModel.findByIdAndUpdate(id, data, { new: true }).lean();
    if (updated) await auditLog("shipments", id, "UPDATE", userId, data);
    return updated;
  },

  async delete(id: string, userId = "system") {
...
export const customerRepo = {
  ...
  async update(id: string, data: any, userId = "system") {
    ...
    if (updated) await auditLog("customers", id, "UPDATE", userId, data);
    return updated;
  },
...
export const trackingRepo = {
  ...
  async addEvent(shipmentId: string, event: any, userId = "system") {
...
export const emailRepo = {
  async getAll(shipmentId?: string) {
    if (shipmentId) return EmailModel.find({ shipment_id: shipmentId }).sort({ sent_at: -1 }).lean();
    return EmailModel.find().sort({ sent_at: -1 }).limit(100).lean();
  },
  async log(data: any, userId = "system") {
...
export const documentRepo = {
  ...
  async updateStatus(id: string, status: string, userId = "system") {
...
export const auditRepo = {
  async getAll(tableName?: string, limit = 100) {
...
export const quotationRepo = {
  async getAll(filters?: { status?: string; customerId?: string }) {
    const query: any = {};
    if (filters?.status) query.status = filters.status;
  ...
  async update(id: string, data: any, userId = "system") {
    ...
    if (updated) await auditLog("quotations", id, "UPDATE", userId, data);
    return updated;
  },
  async delete(id: string, userId = "system") {
...
export const invoiceRepo = {
  ...
  async update(id: string, data: any, userId = "system") {
    ...
    if (updated) await auditLog("invoices", id, "UPDATE", userId, data);
    return updated;
  },
  async delete(id: string, userId = "system") {
...
export const mailLogRepo = {
  async getAll(filters?: { classification?: string; direction?: string; processed?: boolean }, limit = 100) {
    const query: any = {};
    if (filters?.classification) query.classification = filters.classification;
    if (filters?.direction) query.direction = filters.direction;
    if (filters?.processed !== undefined) query.processed = filters.processed ? 1 : 0;
  ...
  async markProcessed(id: string, classification: string, shipmentId?: string) {
    const update: any = { processed: 1, processed_at: new Date(), classification };
...
export const dsrRepo = {
  ...
  async upsert(shipmentId: string, data: any) {
...
export const userRepo = {
  ...
  async updateRole(id: string, role: string, userId = "system") {
...
export const activityRepo = {
  async getAll(module?: string, limit = 200) {
    const query = module ? { module } : {};
    return ActivityLogModel.find(query).sort({ occurred_at: -1 }).limit(limit).lean();
  },
  async log(userId: string, action: string, module: string, details?: object, ipAddress?: string) {
