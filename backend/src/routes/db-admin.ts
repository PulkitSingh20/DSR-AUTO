...
dbAdminRoutes.get("/stats", async (_req, res: Response) => {
...
dbAdminRoutes.get("/table/:name", async (req: AuthRequest, res: Response) => {
  ...
  if (search && ["Shipment", "Customer", "Email"].includes(modelName)) {
    ...
    if (modelName === "Customer") query.name = s;
    else if (modelName === "Email") query.subject = s;
    else query.shipper = s;
  }
...
// ─── Edit a single row ───────────────────────────────────────────────────────
dbAdminRoutes.patch("/table/:name/:id", async (req: AuthRequest, res: Response) => {
  ...
  if (!modelName || !mongoose.models[modelName]) return res.status(400).json({ error: "Unknown table" });
  if (name === "audit_log") return res.status(403).json({ error: "Audit log is read-only" });
...
// ─── Delete a row ─────────────────────────────────────────────────────────────
dbAdminRoutes.delete("/table/:name/:id", async (req: AuthRequest, res: Response) => {
  const { name, id } = req.params;
  const modelMap: Record<string, string> = {
    "shipments": "Shipment", "customers": "Customer", "tracking_events": "TrackingEvent",
    "emails": "Email", "documents": "Document", "reminders": "Reminder",
...
dbAdminRoutes.post("/query", (req: AuthRequest, res: Response) => {
  res.status(400).json({ error: "Raw SQL queries are not supported in MongoDB." });
});

// ─── Audit log ───────────────────────────────────────────────────────────────
dbAdminRoutes.get("/audit", async (req: AuthRequest, res: Response) => {
...
dbAdminRoutes.get("/shipments/stats", async (_req, res: Response) => {
