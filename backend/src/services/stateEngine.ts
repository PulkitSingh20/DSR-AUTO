...
export interface TransitionTrigger {
  type: "reminder" | "email" | "dsr_update" | "dsr_close" | "customer_tag" | "generate_job_number";
  title?: string;
  days?: number;
  template?: string;
  to?: string;
...
const STAGE_TRIGGERS: Record<string, TransitionTrigger[]> = {
...
export async function transitionShipment(
  ...
  const shipment = await ShipmentModel.findById(shipmentId).lean() as any;
  if (!shipment) {
  ...
  let timestamps: Record<string, string> = typeof shipment.stage_timestamps === "object" ? shipment.stage_timestamps || {} : {};
      ...
      switch (trigger.type) {
        ...
        case "dsr_update":
          if (trigger.field && trigger.value) {
            ...
            if (existing) {
              await DsrRecordModel.updateOne(
                { shipment_id: shipmentId },
...
export async function getNextStageAfterBooking(customerId?: string): Promise<ShipmentStage> {
  if (!customerId) return "job_opened";
  const customer = await CustomerModel.findById(customerId).lean() as any;
  if (!customer) return "job_opened";
...
export function getStageInfo(stage: string) {
  const index = SHIPMENT_STAGES.indexOf(stage as ShipmentStage);
  return {
    stage,
    index,
