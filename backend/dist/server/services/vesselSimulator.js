...
// ── Helpers ───────────────────────────────────────────────────────────────────
function deriveStatus(speed, heading) {
    if (speed === 0 || heading === 511)
        return "delayed";
    if (speed < 2)
        return "delayed";
    return "nominal";
...
function startAISStream(apiKey, onVessels) {
...
async function fetchFromAISHub() {
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(`⚠️  AISHub ${res.status}`);
            return null;
        }
        ...
        const vessels = records
            .filter((v) => v.LATITUDE !== 0 && v.LONGITUDE !== 0 && v.NAME?.trim())
            .slice(0, 60)
            .map((v) => ({
            id: `AIS-${v.MMSI}`,
            mmsi: String(v.MMSI),
...
async function fetchFromMarineTraffic() {
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(`⚠️  MarineTraffic ${res.status}`);
            return null;
        }
        const raw = await res.json();
        ...
        if (!records.length)
            return null;
        const vessels = records.slice(0, 60).map((v) => ({
            id: `MT-${v.MMSI}`,
            mmsi: String(v.MMSI),
            imo: v.IMO ? String(v.IMO) : undefined,
    ...
    async start(broadcast) {
        ...
        this.intervalId = setInterval(() => {
            this.vessels = this.vessels.map(v => {
                ...
                const status = roll > 0.98 ? "critical" : roll > 0.95 ? "delayed" : "nominal";
