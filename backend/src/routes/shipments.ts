...
shipmentRoutes.get("/:id", async (req, res) => {
  ...
  const stageInfo = getStageInfo((shipment as any).status);
  res.json({ ...shipment as object, trackingHistory: tracking, stageInfo });
});
...
shipmentRoutes.get("/:id/timeline", async (req, res) => {
  ...
  if (!timeline) return res.status(404).json({ error: "NOT_FOUND" });
  res.json({ timeline });
});

// POST /api/shipments
shipmentRoutes.post("/", async (req: any, res) => {
...
shipmentRoutes.patch("/:id", async (req: any, res) => {
...
shipmentRoutes.post("/:id/transition", async (req: any, res) => {
  const userId = getAuth(req)?.userId || "api";
  ...
  if (target_stage === "auto_after_booking") {
    const shipment = await shipmentRepo.getById(req.params.id) as any;
    if (!shipment) return res.status(404).json({ error: "NOT_FOUND" });
...
shipmentRoutes.patch("/:id/status", async (req: any, res) => {
  const userId = getAuth(req)?.userId || "api";
...
shipmentRoutes.delete("/:id", async (req: any, res) => {
  const userId = getAuth(req)?.userId || "api";
...
shipmentRoutes.post("/:id/tracking", async (req: any, res) => {
  const userId = getAuth(req)?.userId || "api";
