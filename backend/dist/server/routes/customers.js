...
customerRoutes.patch("/:id", async (req, res) => {
    const userId = getAuth(req)?.userId || "api";
...
customerRoutes.patch("/:id/tag", async (req, res) => {
    const userId = getAuth(req)?.userId || "api";
    const { customer_tag } = req.body;
    if (!customer_tag)
        return res.status(400).json({ error: "customer_tag required" });
...
customerRoutes.patch("/:id/kyc", async (req, res) => {
    const userId = getAuth(req)?.userId || "api";
    const { kyc_status } = req.body;
    if (!kyc_status)
        return res.status(400).json({ error: "kyc_status required" });
