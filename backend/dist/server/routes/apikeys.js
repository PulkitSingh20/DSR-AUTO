...
apiKeyRoutes.post("/keys", authMiddleware, (req, res) => {
...
apiKeyRoutes.get("/keys", authMiddleware, (req, res) => {
    if (!req.keyData?.permissions?.includes("admin")) {
...
apiKeyRoutes.delete("/keys/:id", authMiddleware, (req, res) => {
    if (!req.keyData?.permissions?.includes("admin")) {
        return res.status(403).json({ error: "FORBIDDEN" });
    }
...
apiKeyRoutes.get("/verify", authMiddleware, (req, res) => {
    res.json({
        valid: true,
        keyInfo: {
            id: req.keyData.id,
...
apiKeyRoutes.get("/usage", authMiddleware, (req, res) => {
